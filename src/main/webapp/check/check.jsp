...
var enGrid = new App.ux.ExportGridPanel({
		title:'<%="1".equals(sEnType)?"新增企业":"所有企业"%>',
		store: enDs,
	    cm: cm,
	    frame:false,
	    stripeRows: true,
		...
		,{
            ...
            handler : function(){
		        ...
		        if(!records||records.length<1){
					Ext.Msg.alert("提示","请先选择要接受的企业!");
					return;
				}
		        Ext.MessageBox.confirm('确认', '接受操作后，企业不再属于新增企业，是否接受?', function(btn){
					if(btn == 'yes') {// 选中了是按钮
						var ens = new Array();
						...
						CheckHandler.acceptEn(ens.join(),1,function(data){
...
function showTreeWin(cVal,fldName,fldMc,mapBM,isRTK){
	...
	treeWin.onSelect = function(value){
		...
		cText = tmpText.substring(0,tmpText.indexOf("("));
		//根据是否能影响入退库显示入退库窗体。
    	if(isRTK == 1){
...
function showCheckInfoWin(isrtk,fldName,oldVal,newVal,newValDesc){
	...
	rtkForm.getForm().findField("remark").setValue("");
	rtkForm.getForm().findField("affectMode").setValue(false);
	if(isrtk==1){
		//启用，默认值
	    ...
	    handler:function(){
	    	if(required('filepath')!='success'){
	  		...
	  		if (excelForm.getForm().isValid()) {  
				Ext.Msg.wait("正在导入...");
				var d_type = excelForm.getForm().findField("type").getGroupValue(); 
				excelForm.getForm().submit({
		       		...
		       		success: function(form, action) {
						...
						if(obj&&obj.infos){
							...
							var m = obj.infos.msg;
							if(obj.infos.noMatchCount){
								m = m + "<a target='_blank' href='";
								var rptID= "_sys_NOTMATCHBYMC";
								if(d_type=="sh"){
									rptID = "_sys_NOTMATCHBYSH";
								}
								var expUrl="getRptData.query?doType=toExcel&ONLYEXP=1&eformat=0&rptID="+rptID;
								m = m+ expUrl+"' style='cursor:pointer;text-decoration:underline;'>未匹配信息</a>"+obj.infos.noMatchCount+"条。";
							}
							if(obj.infos.bjCount){
								m = m+"<br/>有"+obj.infos.bjCount+"家<a href='";
								var expUrl="getRptData.query?doType=toExcel&ONLYEXP=1&eformat=0&rptID=_sys_NOTBJENS'";
						...
						if(result&&result.columnModel){//模板重新加载
							...
							if(result.columnModel){
								...
								cstore.on("beforeload",function(){
									cstore.baseParams.enType=<%=sEnType%>;
