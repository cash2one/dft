	...
	public void exportExcel(Map model,HttpServletRequest request, HttpServletResponse response){
		try {
			...
			if (StringUtils.isNotBlank(title)) {
				...
				titleStyle.setFillForegroundColor(IndexedColors.LEMON_CHIFFON
						.getIndex());
				// 设置背景色
				// Color.LEMON_CHIFFON
			...
			if(StringUtils.isNotBlank(subTitle)){
				...
				stFont.setBoldweight(HSSFFont.BOLDWEIGHT_BOLD);

				CellStyle stStyle = workbook.createCellStyle();
				stStyle.setFont(stFont);
				stStyle.setAlignment(HSSFCellStyle.ALIGN_CENTER);
				stStyle.setFillForegroundColor(HSSFColor.LEMON_CHIFFON.index);
				stStyle.setFillPattern(HSSFCellStyle.SOLID_FOREGROUND);
			...
			headerStyle.setFillForegroundColor(IndexedColors.GREY_25_PERCENT
					.getIndex());
			headerStyle.setFillForegroundColor(IndexedColors.YELLOW.getIndex());// 设置背景色黄色
			...
			if(StringUtils.isNotBlank(foot)){
				...
				ftStyle.setAlignment(HSSFCellStyle.ALIGN_CENTER);
				ftStyle.setFillForegroundColor(HSSFColor.LEMON_CHIFFON.index);
				ftStyle.setFillPattern(HSSFCellStyle.SOLID_FOREGROUND);
